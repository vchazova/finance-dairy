...
// PATCH /api/dictionaries/payment_types/[id] - update payment type (owner only).
export async function PATCH(req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const message = err?.message ?? "Failed to update payment type";
    const status = err?.status ?? (message === "Unauthorized" ? 401 : message === "Forbidden" ? 403 : 500);
    return NextResponse.json({ ok: false, message }, { status });
  }
}

// DELETE /api/dictionaries/payment_types/[id] - delete payment type (owner only).
export async function DELETE(req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const message = err?.message ?? "Failed to delete payment type";
    const status = err?.status ?? (message === "Unauthorized" ? 401 : message === "Forbidden" ? 403 : 500);
    return NextResponse.json({ ok: false, message }, { status });
  }
}
