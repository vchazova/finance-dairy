...
export async function GET(req: Request) {
  ...
  try {
    const list = await dictionariesRepo.listPaymentTypes(workspaceId);
...
export async function POST(req: Request) {
  try {
    ...
    const parsed = paymentTypeInsertSchema.safeParse(json);
    if (!parsed.success) {
      return NextResponse.json({ ok: false, message: "Invalid input", issues: parsed.error.issues }, { status: 400 });
    }
    const res = await dictionariesRepo.createPaymentType(parsed.data);
